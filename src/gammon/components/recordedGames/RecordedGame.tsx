...
export function RecordedGame<T>(props: {
    children: JSX.Element
    matchRecord: MatchRecord<T>
    index: number | undefined
} & SelectableStateListeners<T>) {
    const { children, index, onSelect, onSelectLatest, onResumeState, matchRecord } = props;
    const eogRecord = matchRecord.curGameRecord.isEoG ?
        matchRecord.curGameRecord.eogRecord : undefined
    const plyRecords = matchRecord.curGameRecord.plyRecords
    const isLatest = index === undefined || (index < 0 || plyRecords.length <= index);

    // 最新状態の場合、どこも選択されていないので何もしない（onClickも設定しない）
    // そうでなければ、履歴遷移時には選択を解除して最新状態にする
    const doResumeState = isLatest ? undefined : () => {
        // 以下の条件は (!isLatest)と同値なので常にtrueだが、コンパイラの警告を回避するために指定している
        if (index !== undefined && (0 <= index && index < plyRecords.length)) {
    ...
    return (
        <div id={"main"}>
            <div id={"boardPane"}>
                {children}
                <CopyTextButton getText={() => formatMatchRecord(matchRecord)} />
                <div id={"matchRecord"}>
