...
export function redSGAutoOperator(engine: GammonEngine = defaultEngine): SGOperator {
    return {
        ...doNothingOperator,
...
export function whiteSGAutoOperator(engine: GammonEngine = defaultEngine): SGOperator {
    return {
        ...doNothingOperator,
...
export function bothSGAutoOperator(engine: GammonEngine = defaultEngine): SGOperator {
    return {
        operateCheckerPlayRed: doCheckerPlay(engine),
        operateCheckerPlayWhite: doCheckerPlay(engine),
        operateRollRed: executeDoRoll,
...
export function bothCBAutoOperator(engine?: GammonEngine): CBOperator {
    return cbAutoOperator(engine, (_: { isRed: boolean }) => true)
}

function cbAutoOperator(engine: GammonEngine | undefined = defaultEngine, isInPlay: (state: { isRed: boolean }) => boolean): CBOperator {
    return {
        operateCubeAction: (cbDispatcher: CubeGameDispatcher, cbState: CBAction, sgState: SGToRoll) => {
            if (isInPlay(cbState)) {
        ...
        operateCubeResponse: (cbDispatcher: CubeGameDispatcher, cbState: CBResponse, sgState: SGToRoll): boolean => {
            if (isInPlay(cbState)) {
