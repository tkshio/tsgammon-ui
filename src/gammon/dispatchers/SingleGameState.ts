import { AbsoluteBoardState, redViewAbsoluteBoard, whiteViewAbsoluteBoard } from "tsgammon-core/AbsoluteBoardState"
import { AbsoluteMove, makeMoveAbsoluteAsRed, makeMoveAbsoluteAsWhite } from "tsgammon-core/AbsoluteMove"
...
export function openingState(boardState: BoardState, dicePip?: DicePip): SGOpening {
        ...
        doOpening: (openingRoll: DiceRoll) => {
            if (openingRoll.dice1 === openingRoll.dice2) {
                ...
                const boardToPlay = isRedPlayerFirst ?
                    boardState.revert() : boardState

                const inPlayState = isRedPlayerFirst ?
                    inPlayStateRed : inPlayStateWhite
...
function inPlayStateWhiteFromNode(
    ...
    const toPly = buildToPly(boardStateNode.dices, makeMoveAbsoluteAsWhite, false)
...
function buildDoCheckerCommit<TOROLL extends SGToRoll, EOG extends SGEoG>(
    ...
    toRollState: (boardState: BoardState, lastPly: Ply, lastNode: BoardStateNode, revertTo: BoardStateNode) => TOROLL,
    toEoGState: (stakeValue: number, lastPly: Ply, committed: BoardStateNode, revertTo: BoardStateNode) => EOG
...
export function toRollStateRed(boardState: BoardState, lastPly: Ply, lastNode: BoardStateNode, revertTo: BoardStateNode): SGToRollRed {
...
export function toRollStateWhite(boardState: BoardState, lastPly: Ply, lastNode: BoardStateNode, revertTo: BoardStateNode): SGToRollWhite {
    const absBoard = whiteViewAbsoluteBoard(boardState)
    return {
        tag: "SGToRoll",
        boardState,
...
export function eogStateWhite(
    ...
    lastPly: Ply,
    committed: BoardStateNode,
    revertTo: BoardStateNode): SGEoGWhiteWon {
    const board = committed.board
    const eogStatus = board.eogStatus()
...
function buildToPly(dices: Dice[], toAbsMove: (move: Move, invertPos: (pos: number) => number) => AbsoluteMove, isRed: boolean): (boardStateNode: BoardStateNode) => Ply {
    return (boardStateNode: BoardStateNode) => {
        return {
            moves: boardStateNode.lastMoves()
