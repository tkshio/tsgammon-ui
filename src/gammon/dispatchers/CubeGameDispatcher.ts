...
import { CBAction, CBEoG, CBInPlay, CBOpening, CBResponse, CBState, CBToRoll, resultToCBEoG } from "./CubeGameState";
...
export type CubeGameDispatcher = {
    ...
    doEndOfCubeGame: (state: CBState, result: SGResult.REDWON | SGResult.WHITEWON, eogStatus: EOGStatus, isWonByPass: boolean) => void
}
...
export function fill(listeners: Partial<CubeGameListeners>): CubeGameListeners {
    const doNothing: CubeGameListeners = {
        onStartCubeAction: (_: CBAction) => { },
        onAwaitCheckerPlay: (_: CBInPlay) => { },
        onDouble: (_: CBResponse) => { },
        onTake: (_: CBToRoll) => { },
        onSkipCubeAction: (_: CBToRoll) => { },
        onEndOfCubeGame: (_: CBEoG) => { }
    }
    return { ...doNothing, ...listeners }
}

export function decorate(base: Partial<CubeGameListeners>, ...listeners: Partial<CubeGameListeners>[]): CubeGameListeners {
    return listeners.reduce(
        (prev: CubeGameListeners, cur: Partial<CubeGameListeners>) => {
            const { onStartCubeAction, onAwaitCheckerPlay, onDouble, onTake: onAccept, onSkipCubeAction, onEndOfCubeGame } = cur
            return {
                onStartCubeAction: onStartCubeAction ? (nextState: CBAction) => {
                    prev.onStartCubeAction(nextState)
                    onStartCubeAction(nextState)
                } : prev.onStartCubeAction,
                onAwaitCheckerPlay: onAwaitCheckerPlay ? (nextState: CBInPlay) => {
                    prev.onAwaitCheckerPlay(nextState)
                    onAwaitCheckerPlay(nextState)
                } : prev.onAwaitCheckerPlay,
                onDouble: onDouble ? (nextState: CBResponse) => {
                    prev.onDouble(nextState)
                    onDouble(nextState)
                } : prev.onDouble,
                onTake: onAccept ? (nextState: CBToRoll) => {
                    prev.onTake(nextState)
                    onAccept(nextState)
                } : prev.onTake,
                onSkipCubeAction: onSkipCubeAction ? (nextState: CBToRoll) => {
                    prev.onSkipCubeAction(nextState)
                    onSkipCubeAction(nextState)
                } : prev.onSkipCubeAction,
                onEndOfCubeGame: onEndOfCubeGame ? (nextState: CBEoG) => {
...
export function setStateListener(setCBState: (cbState: CBState) => void): CubeGameListeners {
    return {
...
export function cubeGameDispatcher(listeners: Partial<CubeGameListeners>): CubeGameDispatcher {
    return {
        doDouble,
        doSkipCubeAction,
        doTake,
    ...
    function doEndOfCubeGame(state: CBState, sgResult: SGResult.REDWON | SGResult.WHITEWON, eogStatus: EOGStatus, isWonByPass: boolean) {
        if (listeners.onEndOfCubeGame) {
            const nextState: CBEoG = resultToCBEoG(state.cubeState, sgResult, eogStatus, isWonByPass)
            listeners.onEndOfCubeGame(nextState)
        }
    }
}
