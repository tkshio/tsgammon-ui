...
export function cubefulSGListener(listeners: Partial<SingleGameListeners>, cbState: CBState, dispatcher: CubeGameDispatcher) {
...
function asSGListeners(state: CBState, cbDispatcher: CubeGameDispatcher) {
    return {
        ...
        onEndOfGame: (sgEoG: SGEoG) => {
            cbDispatcher.doEndOfCubeGame(state, sgEoG.result, sgEoG.eogStatus, false)
        },
    }
}
