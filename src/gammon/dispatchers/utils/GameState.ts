...
import { cbActionRed, cbActionWhite, cbEoGRed, cbEoGWhite, cbInPlayRed, cbInPlayWhite, cbOpening, cbResponseRed, cbResponseWhite, CBState, cbToRollRed, cbToRollWhite } from "../CubeGameState";
import { eogStateRed, eogStateWhite, inPlayStateRed, inPlayStateWhite, openingState, SGState, toRollStateRed, toRollStateWhite } from "../SingleGameState";

export enum GameStatus {
    OPENING, INPLAY_RED, INPLAY_WHITE, CUBEACTION_RED, CUBEACTION_WHITE, CUBERESP_RED, CUBERESP_WHITE, TOROLL_RED, TOROLL_WHITE,
    EOG_REDWON, EOG_WHITEWON
}
...
export type GameState = {
    ...
    gameStatus:
    GameStatus.TOROLL_RED | GameStatus.TOROLL_WHITE |
    GameStatus.CUBEACTION_RED | GameStatus.CUBEACTION_WHITE |
    GameStatus.CUBERESP_RED | GameStatus.CUBERESP_WHITE;
    ...
    gameStatus:
    GameStatus.EOG_REDWON | GameStatus.EOG_WHITEWON;
...
export function toSGState(gameState: GameState = {}): SGState {
    if (gameState.gameStatus === undefined) {
        return openingState(boardState(gameState.absPos ?? standardConf.initialPos))
    }
    switch (gameState.gameStatus) {
        ...
        case GameStatus.CUBERESP_WHITE: {
            ...
            return toRollStateRed(lastStateNode.board.revert(), gameState.lastPly ?? { moves: [], dices: [], isRed: false }, lastStateNode, lastStateNode)
        }
        case GameStatus.CUBEACTION_WHITE:
        case GameStatus.TOROLL_WHITE:
        case GameStatus.CUBERESP_RED: {
            ...
            return toRollStateWhite(lastStateNode.board.revert(), gameState.lastPly ?? { moves: [], dices: [], isRed: true }, lastStateNode, lastStateNode)
        }
        case GameStatus.INPLAY_RED:
            return inPlayStateRed(boardState(gameState.absPos).revert(), gameState)
        case GameStatus.INPLAY_WHITE:
            return inPlayStateWhite(boardState(gameState.absPos), gameState)
        case GameStatus.EOG_REDWON: {
            const lastBoardState = boardState(gameState.absPos).revert()
            const lastStateNode = nodeWithEmptyDice(lastBoardState)
            return eogStateRed(1,
